...
class TeacherConfig(BaseConfig):
    ...
    All teacher models will be optimized with `torch.optim.AdamW`. Optimization
    ...
    def __init__(
        ...
        if not ptrain_ver:
            raise ValueError(
                '`ptrain_ver` must not be empty string.'
            )

        self.ptrain_ver = ptrain_ver

    def __iter__(self) -> Generator[
            Tuple[str, Union[float, int, str]], None, None
    ]:
        attrs = list(super().__iter__()) + [
            ('ptrain_ver', self.ptrain_ver)
        ]
