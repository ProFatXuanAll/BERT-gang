...
class BoolQ(Dataset):
    ...
    def load_distill(experiment_name: str) -> List[Sample]:
        ...
        This is a heavy IO method and might required lots of memory since
        dataset might be huge. BoolQ distillation dataset must be generated by
        some previous experiment. See BoolQ document in
        'project_root/doc/fine_tune_boolq.md' for generating distillation
        dataset details.

        Args:
            experiment_name:
                Name of the BoolQ fine-tune experiment generated by
                `fine_tune.config.BaseConfig.experiment_name()`.

        Raises:
            FileNotFoundError:
                When BoolQ distillation dataset file does not exist.

        Returns:
            A list of samples of BoolQ distillation dataset.
        """
        ...
        try:
            ...
            raise FileNotFoundError(
                f'BoolQ distillation experiment {experiment_name} does not ' +
        ...
        for line in tqdm(
                jsonlines,
                desc=f'Loading BoolQ distillation experiment {experiment_name}'
        ):
            # Skip empty line.
            if line == '':
                continue
    ...
    def save_for_distill(
        ...
        r"""Save logits for BoolQ distillation.
