...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset: str):
        ...
        # Load model generated logits.
        else:
            logger.info(
                'Start loading task %s distillation dataset %s',
                self.__class__.__name__,
                dataset
            )
            self.dataset = self.__class__.load_distill(dataset)
            logger.info(
                'Finish loading task %s distillation dataset %s.',
                self.__class__.__name__,
                dataset
            )
    ...
    def load_distill(experiment_name: str) -> List[Sample]:
        r"""Load distillation dataset into memory.

        This is a heavy IO method and might required lots of memory since
        dataset might be huge. All distillation dataset must be generated by
        some previous experiment. See task document in 'project_root/doc/' for
        generating distillation dataset details.

        Args:
            experiment_name:
                Name of the experiment generated by
                `fine_tune.config.BaseConfig.experiment_name()`.

        Raises:
            FileNotFoundError:
                When distillation dataset file does not exist.

        Returns:
            A list of samples of the distillation dataset.
        """
        raise NotImplementedError(
            'Missing static method `load_distill`.'
        )
    ...
    def update_logits(
        ...
        r"""Update logits by index.

        Logits are used to produce prediction. Teacher model's logits must
        first be normalized using softmax so it can be used as soft-target.

        Args:
            index:
                Sample index of dataset.
            logits:
                Model output's unnormalized logits.

        Raises:
            IndexError:
                `self.dataset` index out of range.
        """
        self.dataset[index]['logits'] = logits
    ...
    def save_for_distill(
        ...
        r"""Save logits along with samples for distillation.
        ...
        """
        raise NotImplementedError(
            'Missing method `save_for_distill`.'
        )
