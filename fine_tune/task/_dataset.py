...
class ContrastDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Sample, int, List[int]]:
        ...
        This method is required by `torch.utils.data.Dataset` and
        `torch.utils.data.Dataloader`. We will return a Tuple which contains
        a `Sample` object, its index and relative negative sample indices.
        ...
        Returns
        -------
        Tuple[Sample, int, List[int]]
            Tuple contains a `Sample` object, its index and
            relative negative sample indices.
    ...
    @staticmethod
    def create_collate_fn() -> ContrastCollateFn:
        r"""Create `collate_fn` used by `torch.utils.data.Dataloader`.
        ...
        CollateFn
            `collate_fn` function used by `torch.utils.data.Dataloader`.
        """
        def collate_fn(batch: List[Tuple[Sample, int, List[int]]]) -> ContrastCollateFnReturn:
            r"""Return a batch of:
            1. text
            2. text pair
            3. label
            4. index positive sample
            5. indices of negative samples

            Parameters
            ----------
            batch : List[Tuple[Sample, int, List[int]]]
                A batch of data sample and relative index
                or indices of postivie and negative sample.
            ...
            # When input text is consist of only 1 sequence.
            if batch[0][0]['text_pair'] is None:
                text_pair = None
                for item in batch:
                    # item is a single tuple.
                    # item[0]: `Sample`.
                    # item[1]: positive index (int).
                    # item[2]: negative indices (List[int]).
                    text.append(item[0]['text'])
                    label.append(item[0]['label'])
                    p_indices.append(item[1])
                    n_indices.append(item[2])
            # When input text is consist of 2 sequences.
            else:
                for item in batch:
                    # item is a single tuple.
                    ...
                    # item[2]: negative indices (List[int]).
                    text.append(item[0]['text'])
                    text_pair.append(item[0]['text_pair'])
                    label.append(item[0]['label'])
                    p_indices.append(item[1])
                    n_indices.append(item[2])
